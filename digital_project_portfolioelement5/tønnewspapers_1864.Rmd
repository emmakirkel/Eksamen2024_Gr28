---
title: "Tønder newspapers concerning Dybbøl 1864"
...
Our data is collected from the digital public data from the Royal Danish Library's newspaper collection on mediestream.dk. The data includes 6 hits from one newspaper, Vestslesvigsk Tidende. 
...
# Loading data from articles about Dybbøl in Tønder newspapers from 1864

On mediestream.dk we have used the seach string: "dybbøl" AND py:1864 AND lplace:Tønder. To export this data to a CSV-file we have used an API represented in the Swagger UI by this link:
...
```{r}
link <- "http://labs.statsbiblioteket.dk/labsapi/api/aviser/export/fields?query=%22dybb%C3%B8l%22%20AND%20py%3A1864%20AND%20lplace%3AT%C3%B8nder&fields=link&fields=recordID&fields=timestamp&fields=pwa&fields=fulltext_org&fields=familyId&fields=lplace&fields=location_name&max=-1&structure=header&structure=content&format=CSV"
```

# Loading the data into R and creating an object with the data

```{r}
tøn_dybbøl_1864 <- read_csv(link)
```
...
```
We also want to check the metadata on which newspaper the articles derive from
...
```{r}
tidy_tøn_dybbøl_1864 <- tøn_dybbøl_1864 %>% 
...
```{r}
top100_tøn_dybbøl_1864<-tidy_tøn_dybbøl_1864 %>% 
  anti_join(stopord_1800) %>% 
  anti_join(stopword_extra) %>% 
  count(word, sort = TRUE) %>% 
...
```{r}
ggplot(data = top100_tøn_dybbøl_1864, aes(label = word, size = n)) +
...
```{r}
fulltext_tidy_recordID<-tidy_tøn_dybbøl_1864 %>%
...
Now we would like to make a new dataframe which includes these two new columns with the full text excluding stopwords, the sentiment for each publication and our original dataframe (tøn_dybbøl_1864) 

We do this by using the left_join()-function

```{r}
sentiments_1864 <- tøn_dybbøl_1864 %>% 
  left_join(sentiment_text_recordID)
```

Now we have a new dataframe (sentiments_1864) which we can use to make a ggplot-visualization of the change in the newspapers sentiment over time
...
We also found it would be interesting to figure out the mean sentiment of all the articles combined

Therefor we are again creating a new dataframe where we are implementing our stopword-lists and group the data by familyId this time, which is an indicator for the newspaper. This is done by using both code we used before but also new code where we use the paste and collapse-functions

```{r}
fulltext_tidy_familyId<-tidy_tøn_dybbøl_1864 %>%
  anti_join(stopord_1800, by = c("word"="word")) %>%
  anti_join(stopword_extra,by=c("word"="word")) %>% 
  group_by(familyId) %>% 
...
Now we are performing the sentiment-analysis and creating a new dataframe which includes both the newspaper (familyId), the full text stopwords excluded (fulltext_stopwords) and the sentiment for the newspaper (fulltext_sentiment)
